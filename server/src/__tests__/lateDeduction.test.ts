...
describe('lateModel', () => {
  ...
  it('charges exactly one started hour at 60 minutes late (second hour not started)', () => {
    const days = [2, 4, 8, 10].map((n) => ({ date: day(n), lateMinutes: 60 }));
    const m = lateModel(days, 3_600_000, WD);
    expect(m.rows[3].amount).toBe(LATE_RATE);
  });
  ...
  it('treats arrival more than 2 hours after grace as a fully unpaid day', () => {
    // 09:30 + 130 min = 11:40, i.e. the "arrive at 11:50" case from the brief
    const days = [2, 4, 8, 10].map((n) => ({ date: day(n), lateMinutes: 140 }));
    const m = lateModel(days, 4_400_000, WD);
    expect(m.rows[3].unpaid).toBe(true);
    expect(m.rows[3].amount).toBe(Math.round(4_400_000 / WD));
  });

  it('values an unpaid day at basic / actual-working-days, so it tracks the month (20 vs 23)', () => {
    const days = [2, 4, 8, 10].map((n) => ({ date: day(n), lateMinutes: 140 }));
    const short = lateModel(days, 4_400_000, 20); // e.g. February
    const long = lateModel(days, 4_400_000, 23); // a 23-weekday month
    expect(short.dailyRate).toBe(220_000);
    expect(long.dailyRate).toBe(Math.round(4_400_000 / 23)); // ~191,304
    expect(short.rows[3].amount).toBe(220_000);
    expect(long.rows[3].amount).toBe(Math.round(4_400_000 / 23));
    expect(short.rows[3].amount).toBeGreaterThan(long.rows[3].amount);
  });

  it('falls back to WORKING_DAYS_PER_MONTH when no working-day count is supplied', () => {
    const days = [2, 4, 8, 10].map((n) => ({ date: day(n), lateMinutes: 140 }));
    const m = lateModel(days, 4_400_000);
    expect(m.dailyRate).toBe(Math.round(4_400_000 / WORKING_DAYS_PER_MONTH));
    expect(m.rows[3].amount).toBe(Math.round(4_400_000 / WORKING_DAYS_PER_MONTH));
  });
